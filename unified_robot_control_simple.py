#!/usr/bin/env python3
"""
Simple Unified Robot Control System
Based on the working interactive_robot_control.py with added MIR integration

Features:
- Working UR robot control with original proven logic
- Simple MIR pause/resume integration
...
try:
    ...
    print("⚠️  MIR control not available - continuing with UR only")
    MIR_AVAILABLE = False

class SimpleUnifiedRobotController:
    """Simple unified robot controller with working UR logic + MIR integration"""
    
    def __init__(self, robot_ip: str = "192.168.1.6", mir_ip: str = "mir.com", functions_dir: str = "functions", control_dir: str = "control"):
        ...
        # Speed control (same as original)
        ...
        # Initialize MIR if available
        self._init_mir_control()
        
    def _init_mir_control(self):
        """Initialize MIR control system"""
        if not MIR_AVAILABLE or self.mir_ip == "offline":
            print("⚠️  MIR control disabled")
            self.mir_enabled = False
            return
    ...
    def add_function(self, function_name: str, speed: Optional[float] = None, pause_mir: bool = None):
        """Add function to execution queue with optional MIR coordination"""
        commands = self.load_function(function_name)
        if commands is None:
            print(f"❌ Function '{function_name}' not found")
            return False
        ...
        print(f"✅ Added function '{function_name}' to queue (speed: {use_speed} m/s, MIR pause: {mir_pause})")
        return True
    ...
    def _process_external_command(self, command: str):
        ...
        try:
            ...
            elif cmd == "collab" and len(parts) > 1:
                function_name = parts[1]
                speed = float(parts[2]) if len(parts) > 2 else None
                self.add_function(function_name, speed, pause_mir=True)
                self._write_response(f"Collaborative function '{function_name}' added")
    ...
    def worker_function(self):
        ...
        while self.running:
            ...
            try:
                function_name, commands, base_speed, pause_mir = self.function_queue.get(timeout=1.0)
            ...
            try:
                ...
                if command in ['quit', 'exit', 'q']:
                    ...
                    if len(parts) > 1:
                        ...
                        print("❌ Usage: add <function_name> [speed] [pause_mir]")
                elif command == 'collab':
                    if len(parts) > 1:
                        function_name = parts[1]
                        speed = float(parts[2]) if len(parts) > 2 else None
                        self.add_function(function_name, speed, pause_mir=True)
                        print(f"🤝 Collaborative function '{function_name}' added (MIR will auto-pause)")
                    else:
                        print("❌ Usage: collab <function_name> [speed]")
                elif command == 'mir':
                    if len(parts) > 1:
                        ...
                        if mir_cmd == 'pause':
                            ...
                            print("❌ Usage: mir <pause|resume|status|auto <true/false>>")
                    else:
                        print("❌ Usage: mir <pause|resume|status|auto <true/false>>")
    ...
    def show_help(self):
        ...
        print("""
🤖 Simple Unified Robot Control Commands:

UR ROBOT COMMANDS:
  add <func> [speed] [mir_pause] - Add UR function (mir_pause: true/false)
  collab <func> [speed]          - Add UR function with MIR auto-pause
  list, l                        - List available UR functions
  speed <value>                  - Set UR base speed (0.01-1.0 m/s)
  multiplier <val>               - Set UR speed multiplier (0.1-5.0x)
  slow / fast                    - Halve/double current speed multiplier

MIR BASE COMMANDS:
  mir pause                      - Pause MIR base
  mir resume                     - Resume MIR base  
  mir status                     - Show MIR status
  mir auto <true/false>          - Enable/disable auto-pause during UR functions

SYSTEM COMMANDS:
  pause, p                       - Pause entire system
  resume, r                      - Resume entire system
...
EXTERNAL CONTROL:
  ...
  Available commands: pause, resume, stop, status, speed <val>, multiplier <val>
                     add <func> [speed] [mir_pause], ur_add <func> [speed] [mir_pause]
                     mir_pause, mir_resume, mir_status, mir_auto <true/false>
                     collab <func> [speed], slow, fast
...
def main():
    parser = argparse.ArgumentParser(description="Simple Unified Robot Control System")
    parser.add_argument("--robot-ip", default="192.168.1.6", help="UR Robot IP address")
    ...
    controller = SimpleUnifiedRobotController(
